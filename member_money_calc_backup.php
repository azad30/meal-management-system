                        ...
                        <div class="table-responsive">
						 ...
						 <?php
							if(isset($_POST['calc'])){
							  ...
							  $register = "INSERT INTO member_total_meal(member_id,total_meal,meal_rate,tm_into_meal_rate,total_market,give_or_got_money) VALUES ('$member','$totalmeal','$mealrate','$mul','$market','$giveorgot')";
						  ...
						  <table class="table table-bordered table-responsive">
							<thead>
							  <tr>
								...
								<th>Total Meal</th>
								<th>Meal Rate</th>
								<th>Meal into Meal rate</th>
								<th>Total Market</th>
								<th>Give or Got money</th>
							  </tr>
							</thead>
                                <?php
                                $mmc = "SELECT member_total_meal.member_id,member_total_meal.total_meal,member_total_meal.meal_rate,member_total_meal.tm_into_meal_rate,member_total_meal.total_market,member_total_meal.give_or_got_money,member.name
                                            FROM member_total_meal,member
                                            WHERE member.id=member_total_meal.member_id";
                                $runmmc = mysqli_query($connect,$mmc);
                                while($row=mysqli_fetch_assoc($runmmc)){
									$tmm = $row['tm_into_meal_rate'];
									$sm   =$row['total_market'];
                                 ?>
							<tbody>
							  <tr>
								<td><?php echo $row['name']; ?></td>
								  ...
								  <td>
								      <?php
									   if($sm>$tmm){
										  echo $row['name'].'&nbsp'.'Got'.'&nbsp' .$row['give_or_got_money'].'&nbsp'.'Taka';
										  echo '&nbsp'.'<i class="fa fa-smile-o" aria-hidden="true" style="font-size:20px;"></i>';
									   }else{
										   echo $row['name'].'&nbsp'.'Give'.'&nbsp' .$row['give_or_got_money'].'&nbsp'.'Taka';
								  ...
								  <option value="<?php echo $row['id'];?>"><?php echo $row['name']; ?></option>
								   ...
								   <div class="form-group">
                                     <label for="mealinput">Total Meal:</label>
                                      <input type="text" name="tm" class="form-control" id="mealinput">
                                   </div>
								    <div class="form-group">
                                     <label for="mealinput">Meal Rate:</label>
                                      <input type="text" name="meal_rate" class="form-control" id="mealinput">
                                   </div>
								     <div class="form-group">
                                     <label for="mealinput">Member total market:</label>
                                      <input type="text" name="mm" class="form-control" id="mealinput">
                                   </div>
